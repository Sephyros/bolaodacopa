...
export default function Home(props: HomeProps) {
  ...
  async function createPoll(event: FormEvent) {
    ...
    try {
      ...
      alert(`Bolão "${pollTitle}" criado com sucesso, o código "${code}" foi copiado para a área de transferência!`)
      setPollTitle('')
    } catch (error) {
      console.log(error)
      alert('Falha ao criar o bolão (veja o log)')
    }
  ...
  return (
    <div className='max-w-[1124px] h-screen mx-auto grid grid-cols-2 gap-28 items-center'>
      ...
      <Image
        src={appPreviewImg}
        alt="Uma foto com dois celulares simulando o aplicativo em funcionamento"
        quality={100}
      />
    </div >
  )
